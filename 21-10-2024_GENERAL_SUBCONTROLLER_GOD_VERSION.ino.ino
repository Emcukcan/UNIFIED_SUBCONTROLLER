...
String FirmwareVer = "23-10-2024 Ver1";
...
void INVERTERCANBUSTASK_CODE( void * pvParameters ) {
  ...
  for (;;) {
    ...
    TemperatureRule = String1Temp < INVTempLim;
    CellVoltageRule = String1MaxCellModbus < INVMaxCellLim;
    MaxSOCRule = String1SOC < INVSOCLimMax;
    MinSOCRule = String1SOC > INVSOCLim;
    SOCOverlookRule = String1SOC > SOCOverlook;
    ...
    if (InverterType == 1) {
      ...
      if (SOCOverlookRule) {
        ...
        if ((String1Voltage < MaximumStringVolt * 0.980 )) {
          MaxSystemVoltageRule = 100;
        }
        else if (String1Voltage >= MaximumStringVolt * 0.980 && String1Voltage < MaximumStringVolt * 0.985) {
          MaxSystemVoltageRule = 50;
        }
        else if (String1Voltage >= MaximumStringVolt * 0.985 && String1Voltage < MaximumStringVolt * 0.990) {
          MaxSystemVoltageRule = 25;
        }
        else if (String1Voltage >= MaximumStringVolt * 0.99 && String1Voltage < MaximumStringVolt * 0.995) {
          MaxSystemVoltageRule = 10;
        }
        else if (String1Voltage >= MaximumStringVolt * 0.995 && String1Voltage < MaximumStringVolt * 1) {
      ...
      CalculatedChargeCurrent = MaxCurrent  * MaxSystemVoltageRule * 0.01 * MaxSOCRule;
      CalculatedDischargeCurrent = MaxCurrent * 100 * 0.01 * MinSOCRule;


      if (String1MaxCellModbus < 4.4  && String1MinCellModbus < 4.7 && String1MaxCellModbus > 1.8  && String1MinCellModbus > 1.8) {
        set_maxvoltage(String1MaxCellModbus, String1MinCellModbus, String1SOC, String1SOC, 0, 0x180150F1);
        delay(10);
        set_totalvoltage(String1Voltage , String1Current , CalculatedChargeCurrent, CalculatedDischargeCurrent, 0x180250F1);
        delay(10);
  ...
  for (;;) {
    ...
    if (BoardVersion) {
      if (!FORCE) {
        ...
        // CRITICAL UPDATE
        digitalWrite(5, true); // PRECHARGE IS ALWAYS ON TO ELIMINATE IMBALANCING
        SystemWarning4 = "Precharge is ENABLED ALWAYS";
      ...
      if (!FORCE) {
        ...
        // CRITICAL UPDATE
        digitalWrite(18, true); // PRECHARGE IS ALWAYS ON TO ELIMINATE IMBALANCING
        SystemWarning4 = "Precharge is ENABLED ALWAYS";
...
void BT_CODE( void * pvParameters ) {
  ...
  //String BT_STATION = "ENSUB-"+FirmwareVer+"-" + SerialNumber + "-#" + String(SubID);


  String BT_STATION = "ENSUB-" + FirmwareVer + "-" + String(SubID);
  ...
  Serial.println("Check in list as " + BT_STATION);
  ...
  for (;;) {
    ...
    if (SerialBT.available()) {
      ...
      param_end2 = message.indexOf("#");
      if (param_start2 != -1 && param_end2 != -1) {
        SerialBT.println("MAXCELL:" + String(String1MaxCellModbus) + "/MINCELL" + String(String1MinCellModbus) + "/SOC" + String(String1SOC) +
                         "/SystemVolt" + String(String1Voltage) + "/SystemCurrent" + String(String1Current) +
                         "/ChargeCurrentBA" + String(MaxCurrent) + "/DischargeCurrentBA" + String(MaxCurrent) + "/THRCH" + String(ThrottleCH) + "/THRDS" + String(ThrottleDS) + "#");
        message = "";
      }
...
void MODBUSTASK_CODE( void * pvParameters ) {
  ...
  for (;;) {
    ...
    if (String1MaxCellModbus  < 4.8  && String1MinCellModbus  < 4.8  && String1SOC < 105  && abs(String1Current) < 800 && String1MaxCellModbus  > 2.2 && String1MinCellModbus  > 2.2 && String1Voltage < 1000 && String1Voltage > 40 ) {

      Serial.println("modbus works");
      for (int i = 0; i < ModuleSize; i++) {
...
void analyseNewData() {
  ...
  if (receivedString == "-SHOWSUMMARY") {
    ...
    Serial.println("         INVTempLim Value:" + String(INVTempLim));
    Serial.println("         INVMaxCellLim Value:" + String(INVMaxCellLim));
    Serial.println("         INVSOCLim Value:" + String(INVSOCLim));
  ...
  else if (receivedString.indexOf("SETINVTEMPLIM") != -1 && receivedString.indexOf("#") != -1) {
    SerialProcessor = receivedString.substring(receivedString.indexOf("SETINVTEMPLIM") + 13, receivedString.indexOf("#"));
    INVTempLim = SerialProcessor.toInt();
    Serial.println("INVTempLim:" + String(INVTempLim));
    receivedString = "";
    preferences.begin("my-app", false);
    preferences.putInt("INVTempLim", INVTempLim);
    preferences.end();
  }
  ...
  else if (receivedString.indexOf("SETINVMAXCELL") != -1 && receivedString.indexOf("#") != -1) {
    SerialProcessor = receivedString.substring(receivedString.indexOf("SETINVMAXCELL") + 13, receivedString.indexOf("#"));
    INVMaxCellLim = SerialProcessor.toFloat();
    Serial.println("INVMaxCellLim:" + String(INVMaxCellLim));
    receivedString = "";
    preferences.begin("my-app", false);
    preferences.putInt("INVMaxCellLim", INVMaxCellLim);
    preferences.end();
  }

  //SET INVSOCLIM//////////

  else if (receivedString.indexOf("SETINVSOCLIM") != -1 && receivedString.indexOf("#") != -1) {
    SerialProcessor = receivedString.substring(receivedString.indexOf("SETINVSOCLIM") + 12, receivedString.indexOf("#"));
    INVSOCLim = SerialProcessor.toFloat();
    Serial.println("INVSOCLim:" + String(INVSOCLim));
    receivedString = "";
    preferences.begin("my-app", false);
    preferences.putInt("INVSOCLim", INVSOCLim);
    preferences.end();
  }
